> Superseded by 2026-06-14 SL-first strategy.
> This document is retained as historical context only. It must not be treated as an active implementation queue unless explicitly updated after 2026-06-14.
# 封存文件

已完成或已被 `../../專案總覽.md` (`../../專案總覽.md`) 取代的歷史計畫。**請勿在此更新狀態**，以專案總覽為準。

| 文件 | 內容 | 完成 |
|------|------|------|
| `重構計畫_P0-P6.md` (`重構計畫_P0-P6.md`) | 結構重構 P0–P6 | 2026-06-08 |
| `重構計畫_Phase2.md` (`重構計畫_Phase2.md`) | 研究營運 O1–O6 實作細節 | 2026-06-09 |
| `重構計畫_現況評估.md` (`重構計畫_現況評估.md`) | R 系列研究快照（R4/R6 smoke） | 封存 |
| `VERIFICATION_P4-P5.md` (`VERIFICATION_P4-P5.md`) | P4/P5 驗證報告 | 2026-06-07 |
| `RL_Trading_DQN_PPO_Guide.md` (`RL_Trading_DQN_PPO_Guide.md`) | v10 策略指南（部分過時） | 封存 |
| `macro_universe_separation.md` (`macro_universe_separation.md`) | RL/Flow macro 分離 | 已併入 ARCHITECTURE §6 |
| `SAC_BUFFER_PLAN_v2.md` (`SAC_BUFFER_PLAN_v2.md`) | SAC v2 雙線排程 + P8 規格 | v3 取代 |
