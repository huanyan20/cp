...
1. **R6**：依 O2 分層協議重訓，驗證 R4 reward 是否把 worst-case MDD 壓到 35% 以下
2. 觀測補 rolling vol / rolling Sortino proxy，緩解 POMDP
3. overnight features 維持風控層（O6），不進 RL 預設輸入（R5）
...
- **下一步**：先驗證 R4（透過 O2 分層重訓 R6），再決定是否進入中期架構調整。
