...
"""CMoney multi-account configuration discovery.

Extracted from ``cmoney_rpa.py`` during P6 refactor.

Responsibilities
----------------
- Auto-discover all virtual-trading sub-accounts from the CMoney website.
- Build the account list consumed by the CLI and ``CMoneyRPA`` batch loop.

This module has **no** dependency on ``cmoney_rpa``; it can be imported
independently for configuration inspection or testing.
"""
...
def get_accounts_config() -> list[dict]:
    ...
    if not explicit_aids:
        print("[╰参] .env ゼも笆﹚ AID盢笆祅碔ъ┮Τ眀め...")
        explicit_aids = get_auto_aids(default_cookie)
        if explicit_aids:
            print(f"[╰参] Θъ {len(explicit_aids)} 眀め: {explicit_aids}")
        else:
            print("[╰参] тぃヴ眀め ID")
    ...
    for i in range(1, 21):
        ...
        if not extra_cookie:
            continue
        print(f"[╰参] 盎代肂穦 Cookie {i}笆ъㄤ眀め...")
