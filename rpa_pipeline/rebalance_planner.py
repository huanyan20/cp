...
- ``build_rebalance_plan``  — compute buy/sell lot diffs from current vs target

- ``build_dry_run_diff``    — produce a dry-run diff JSON payload

- ``write_dry_run_diff``    — high-level helper: load signal, query RPA, write JSON

- ``run_signal_file``       — orchestrate a full signal file execution / dry-run

- ``_current_lots_from_rpa`` — extract current lot counts from an RPA instance
...
from rpa_pipeline.signal_validator import _normalize_sid, load_signal, record_signal
