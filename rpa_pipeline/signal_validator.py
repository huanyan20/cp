...
- ``SignalError``           — custom exception for malformed / expired signals

- ``EXECUTION_LOG_FILE``    — path to the execution log (module-level constant)

- ``load_signal``           — parse, validate, and normalise a signal JSON file

- ``record_signal``         — append a signal ID to the execution log

- ``signal_was_executed``   — check whether a signal ID has been logged
