...
1. ~~**M1a–M1c + M1d**~~：obs POMDP · reward r5 · `softmax_temp=1.0` · top-k floor → **r5.1 done**
2. **短跑參考**（30K · 2025H1）：MDD **31.9%** · max holding **18.3%** → [`.research/runs/`](.research/runs/README.md)
3. **M2-smoke 300K**（需確認）：`walk_forward.py --tier smoke --algo sac --cash-mode enabled`
4. **M2-candidate** → promotion（O2 分層）

**停損**：smoke 2025H1 MDD ≥ r4 seed42（37.7%）或單檔 ≥ 50% → 不進 candidate；promotion worst MDD > 38% → RL 路徑暫停。
