...
| R6 candidate / promotion | ❌ 中斷（SAC OOM @ ~128K steps；可續跑） |
...
**已拍板**：overnight → overlay only · with_features 移出主 ranking · R4 待 R6 驗證

```bash
.\env\Scripts\python.exe walk_forward.py --candidates --tier candidate
.\env\Scripts\python.exe walk_forward.py --candidates --tier promotion
.\env\Scripts\python.exe -m sl_pipeline.walk_forward_sl --horizon 5 --allocator rule --gate
...
**R6 中斷（2026-06-09）**：candidate 階段 SAC enabled / seed42 在 **2025H1 訓練 ~128K/150K** 時 **記憶體不足**（`ArrayMemoryError`）。`2024H2` 模型已存（`wf_sac_enabled_model_2024H2_seed42.zip`）。promotion **未開始**。

**續跑（不加 `--overwrite`）**：
```bash
.\env\Scripts\python.exe walk_forward.py --candidates --tier candidate
.\env\Scripts\python.exe walk_forward.py --candidates --tier promotion --overwrite
```
